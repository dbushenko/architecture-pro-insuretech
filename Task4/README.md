...
- WebSocket-ы достаточно сложная, а главное -- хрупкая технология. Проходит не через все прокси, там есть и другие технические сложности, например, при разрыве соединение не восстанавливается автоматически, нет поддержки CORS и т.д. Кроме того, websocket привязывается к конкретному инстансу pod-а, что накладывает на архитектуру дополнительные ограничения и по факту делает архитектуру stateful.
WebSockets в сложных ситуациях подходит для разработки чатов и т.д., где нужна двунаправленная связь.

- Server Sent Events -- протокол значительно проще, работает прямо поверх HTTP/HTTPS, технических ограничений никаких особых нет. Позволяет организовать новостные ленты, т.к. является однонаправленным (от сервера к клиенту). Для нашей задачи подходит идеально.

- Запросы по таймеру -- раз в 3-4 секунды запросить у сервера обновление информации. Является наиболее старым и простым подоходом, удобным, но не слишком "быстрым", т.к. минимальная задержка между обновлениями -- время ожидания таймера. Для нашей задачи также подходит хорошо.
...
Важный момент. Поскольку инстансов *core-app* может быть несколько, необходимо, чтобы каждый из них был в своей собственной уникальной kafka consumer group. Таким образом, через каждый *core-app* будут проходить все сообщения из *broker*. Тот инстанс *core-app*, который держит открытое SSE-соединение с клиентом, получит все сообщения, выберет из них те, что относятся конкретно к нему, и доставит ползователю.
